...
namespace PEASA_App
{
    public partial class AsistenciaEstampados2 : Form
    {
        ...
        public AsistenciaEstampados2(MySQLConnectionManager connectionManager)
        {
            InitializeComponent();
            ...
            dgv_AttendanceList2.AllowUserToAddRows = false;
            dgv_AttendanceList2.CellEndEdit += dgv_AttendanceList2_CellEndEdit;
        }

        private void AsistenciaEstampados2_Load(object sender, EventArgs e)
        {
        ...
        private void dgv_AttendanceList2_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && dgv_AttendanceList2.Columns[e.ColumnIndex].Name == "Attendance")
            {
                DataGridViewComboBoxCell comboBoxCell = dgv_AttendanceList2.Rows[e.RowIndex].Cells["Attendance"] as DataGridViewComboBoxCell;
        ...
        private void MostrarListaTrabajadores()
        {
            try
            {
                DataTable workersData = workersDataBase.GetAttandanceEstampadosList2();

                dgv_AttendanceList2.DataSource = workersData;

                // Verificar si hay trabajadores en la lista antes de agregar la columna de ComboBox
                if (workersData.Rows.Count > 0 && dgv_AttendanceList2.Columns["Attendance"] == null)
                {
                    ...
                    dgv_AttendanceList2.Columns.Add(comboBoxColumn);
                }

                foreach (DataGridViewRow row in dgv_AttendanceList2.Rows)
                {
